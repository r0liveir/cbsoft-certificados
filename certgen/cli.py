...
def _parser() -> argparse.ArgumentParser:
    ...
    generate.add_argument("--sheet", "--data-sheet", dest="sheet", required=True)
...
def _convert_to_pdf(pptx_file: Path, output_file: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="certgen-") as directory:
        profile = (Path(directory) / "lo-profile").as_uri()
        ...
        shutil.copy2(produced, output_file)
...
def _generate(args) -> int:
    ...
    for row_number, row in enumerate(rows, start=2):
        try:
            contexts.append(build_context(variables, row, mappings))
        except CertificateError as error:
            errors.append(f"{args.sheet}!{row_number}: {error}")
    if errors:
        raise CertificateError("Validation failed; no certificates were generated:\n- " + "\n- ".join(errors))

    args.output.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="certgen-pptx-") as directory:
        temp_dir = Path(directory)
        for row_number, context in enumerate(contexts, start=2):
            name = safe_filename(f"{args.sheet}-{row_number - 1}")
            pptx_file = temp_dir / f"{name}.pptx"
            render(args.template, args.slide, context, pptx_file)
            _convert_to_pdf(pptx_file, args.output / f"{name}.pdf")
    print(f"Generated {len(contexts)} PDF certificate(s) in {args.output}")
    return 0
